      ...
      <div className="flex items-center justify-center p-6 sm:p-12">
        <div className="w-full max-w-md space-y-5">
          ...
          <form className="space-y-4">
            ...
            <Input label="Email" type="email" placeholder="you@college.edu.in" />
            <Input label="College Name" placeholder="Your college name" />
            <div className="grid grid-cols-2 gap-3">
              <Select label="Year" options={["1st", "2nd", "3rd", "4th"]} />
              <Select label="Branch" options={["CSE", "ECE", "EEE", "Mech", "Civil", "IT", "Other"]} />
            </div>
...
function Select({ label, options }: { label: string; options: string[] }) {
  return (
    <label className="block">
      <span className="mb-1.5 block text-sm font-medium">{label}</span>
      <select className="w-full rounded-xl border border-border bg-card px-4 py-3 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20">
